...
for db in ${EVENTUATE_DATABASES:-eventuate} ; do
  ...
  if [ ! -z "$EVENTUATE_OUTBOX_TABLES" ] ; then
    for i in $(seq 0 $(($EVENTUATE_OUTBOX_TABLES - 1))) ; do
      EVENTUATE_OUTBOX_SUFFIX=$i envsubst '$EVENTUATE_DATABASE,$EVENTUATE_OUTBOX_SUFFIX' < /additional-scripts/3.create-message-table.sql >> /docker-entrypoint-initdb.d/3.initialize-database.sql
    done
  fi

  if [[ "${USE_JSON_PAYLOAD_AND_HEADERS}" == "true" ]]; then
    envsubst '$EVENTUATE_DATABASE' <  /additional-scripts/4.initialize-database-json.sql >> /docker-entrypoint-initdb.d/4.initialize-database-json.sql
    echo "4.initialize-database-json.sql is activated for $EVENTUATE_DATABASE"
  fi

  if [[ "${USE_DB_ID}" == "true" ]]; then
    envsubst '$EVENTUATE_DATABASE' <  /additional-scripts/5.initialize-database-db-id.sql >> /docker-entrypoint-initdb.d/5.initialize-database-db-id.sql
